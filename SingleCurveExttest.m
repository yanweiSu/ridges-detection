...
tic;
for k = 1:3
    [c] = CurveExt(TFRtmp', lambda(k));
    for l = 1:length(c)
        % masking
        TFRtmp( max(c(l)-bw,1):min(c(l)+bw,size(TFRtmp,1)) , l ) = 0;
    end
    cALL = [cALL reshape(c, [], 1)];
end
elapsed(r) = toc;

[~,minIdx] = min(cALL(round(length(c)/3),:));
c_fund = cALL(:,minIdx);
...
curves(:,r) = tfrtic(c_fund);
seg = tt(1)+1 : tt(end);
rmse(r) = norm((tfrtic(c_fund(seg))-IF1_fund(seg)),2)/norm(IF1_fund(seg),2);
fprintf("%d, RMSE = %.2f, time = %.2f\n", r, rmse(r), elapsed(r));

end

curve_SingleRD{am,snrdb} = curves;
RMSE_SingleRD{am,snrdb} = rmse;
time_SingleRD{am,snrdb} = elapsed;

save('./newARMAnoise/curve_SingleRD.mat', 'curve_SingleRD');
save('./newARMAnoise/RMSE_SingleRD.mat', 'RMSE_SingleRD');
save('./newARMAnoise/time_SingleRD.mat', 'time_SingleRD');
