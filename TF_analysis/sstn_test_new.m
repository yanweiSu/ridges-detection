function [STFT,SST1,SST2,SST3,SST4,omega,omega2,omega3,tau2,tau3,phi22p,phi23p,phi24p] = sstn_test_new(s,gamma,sigma,Nfft,bt)
...
% = zeros(neta,nb);
vg = zeros(neta,7); %acá modifiqué el tamaño, poniendo 8 columnas en lugar de 7
vgp = zeros(neta,5); %idem
Y = zeros(neta,4,4);
...
for b=1:nb
    ...
    for i = 0:7
        if Nfft<=n
            ...
            %vg(:,i+1) = tmp(ft);
            vg(:,i+1) = tmp(n/Nfft:n/Nfft:n/2);
        else
            tmp = (fft([x(bt(b):bt(b)+n-1).*(t.^i).*g; zeros(Nfft-n,1)]))/n;
            vg(:,i+1) = tmp(1:Nfft/2);
        end;
    end 
    ...
    for i = 0:5
        if Nfft<=n
            ...
            %vgp(:,i+1) = tmp(ft);
            vgp(:,i+1) = tmp(n/Nfft:n/Nfft:n/2);
        else
            tmp = fft([x(bt(b):bt(b)+n-1).*(t.^i).*gp; zeros(Nfft-n,1)])/n;
            vgp(:,i+1) = tmp(1:Nfft/2);
        end;
    end
    ...
    if Nfft<=n
        ...
        STFT(:,b) = vg(:,1).* exp(1i*pi*(ft-n/Nfft)');
    end;
end
